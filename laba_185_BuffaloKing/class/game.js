...
var GameInfo = function () {
    ...
    var Game = function () {
        ...
        this.lottery = function (userId, nBetSum, jackpot) {
            ...
            // 生成图案，分析结果（结果不满意继续）
            while (true) {
                nHandCards = laba.CreateRandomCardList(gameConfig.GAME_COLORS_DIAMOND, gameConfig.GAME_HAND_CARDS_Column, gameConfig.GAME_HAND_CARDS_Row, gameConfig.GAME_HAND_CARDS_No_Magic_Column_List);
                // 下注倍数
                const bet_num = parseInt(nBetSum / gameConfig.GAME_GOLD_Single);
                // 获取处理结果
                dictAnalyseResult = laba.AnalyseColumnSolt(nHandCards, gameConfig.GAME_MAGIC_CARD_DIAMOND, gameConfig.GAME_FREE_TIMES_CARD_DIAMOND, gameConfig.GAME_HAND_CARDS_LowerLimit, gameConfig.GAME_HAND_CARDS_Column, bet_num, gameConfig.GAME_COMBINATIONS_DIAMOND, gameConfig.GAME_GOLD_Single);
...
function RandomFreeBet(_type) {
    //生成指定范围内随机整数
    let Rand = Math.random();
    let num = parseInt(Rand * 3); //四舍五入
    let bet = [2, 3, 5, 8, 10, 15, 30];
    return bet[_type + num];
}
